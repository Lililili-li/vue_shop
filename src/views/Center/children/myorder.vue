  ...
  <div>
    <div class="order-right">
      <div class="order-content">
        ...
        <div class="orders">
          <table class="order-item" v-for="item in pagedata.records" :key="item.id">
            <thead>
              <tr>
                <th colspan="5">
            ...
            <tbody>
              <tr v-for="(goods,index) in item.orderDetailList" :key="goods.id">
                ...
                <td :rowspan="item.orderDetailList.length" v-if="index === 0" width="13%" class="center">
                  <ul class="unstyled">
                    <li>总金额¥{{goods.orderPrice * goods.skuNum}}.00</li>
                    <li>在线支付</li>
                  </ul>
                </td>
        ...
        <Pagination :pageNo="page" :pageSize = "pagedata.size" :total = "pagedata.total" :continues = '5'  @getpageNo = 'getpageNo' @pageprev = "pageprev" @pagenext = "pagenext" @changesize="changesize"/>
      </div>
      <!--猜你喜欢-->
      <div class="like">
        <h4 class="kt">猜你喜欢</h4>
...
export default {
  ...
  methods: {
    async getpagelist () {
      ...
      const result = await reqpage(this.page, this.limit)
      if (result.code === 200) {
