...
import { reqpayment, reqpaystate } from '@/api'
export default {
  name: 'Pay',
  components: {
    Header,
  ...
  methods: {
    async getpaymentinfo () {
      const result = await reqpayment(this.orderid)
      if (result.code !== 200) {
    ...
    async open () {
      ...
      this.$alert('<img src=' + url + ' />', '微信支付', {
        ...
        showClose: false,
        beforeClose: async (type, done) => {
          if (type === 'confirm') {
            const result = await reqpaystate(this.orderid)
            if (result.code === 200) {
